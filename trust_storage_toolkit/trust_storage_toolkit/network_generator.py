...
class NetworkGenerator:
    ...
    def __init__(self):
        ...
        self.bootnode_generator = BootnodeGenerator(self.crypto_service)
        self.permissions_generator = PermissionsGenerator(self.crypto_service)
        self.private_key_generator = PrivateKeyGenerator(self.crypto_service)
        self.genesis_generator = GenesisGenerator(self.crypto_service, self.contract_service)
    
    def generate_network(self, compose_path: Path, genesis_template_path: Path) -> None:
        """Generate all network configuration files."""
        # Parse configuration
